        ...
        for i in range(b):
            ...
            if self.num_eigenvectors > 0:
                with torch.no_grad():
                    ...
                    ew_detached = ew.squeeze(-1).detach()
                    try:
                        L_idx, L_w = get_laplacian(edge_tuples, ew_detached, normalization="sym", num_nodes=num_n)
                ...
                x_i = torch.cat([node_last[i], pe.detach()], dim=-1)
            else:
                x_i = node_last[i]
...
class EvoBrain_classification(nn.Module):
    ...
    def __init__(self, args, num_classes, device=None, gnn="ssg", num_eigenvectors=16):
        ...
        if args.agg != "concat":
            self.fc = nn.Linear(self.evobrain.feat_target_size, num_classes)
        else:
